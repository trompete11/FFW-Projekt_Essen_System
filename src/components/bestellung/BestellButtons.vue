  ...
  function addFoodItemClick(data: FoodItem) {
    ...
    function calcBtnHeightStyle():string{
      if(site.value !== undefined)
        return "height: " + (65 / site.value.buttons.length) + "vh"
      return ""
    }
...
<template>
  ...
  <div class="row row-radio" :style="calcBtnHeightStyle()" v-for="(row, index) in site?.buttons" v-bind:key="index">
    <div class="col col-radio" v-for="btn in row" v-bind:key="btn.btnData.id">
      <TextFieldBase v-if="btn.btnData.type === 'text'" :btn_data="btn" @change="(data:string) => {item.comment = data}"/>
      <BestellButton
        v-else
        :btn="btn"
        @click="(item) => addFoodItemClick(item)"
        @addClick="$emit('addClick', item)"
        @siteChange="(newSite) => {changeSite(newSite)}"
      />
    </div>
  </div>
</template>
